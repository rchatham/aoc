...
public class Day1 {
    ...
    public static func calculateSum() -> Int {
        return Day1.input.split(separator: "\n").reduce(0) { res, code in
            let int = Day1.getNumber(string: String(code))
            if int > 0 {
                return res + int
            }
            return res
    ...
    // return -1 if cannot find number
    public static func getNumber(string: String) -> Int {
        var firstDigit: Int = -1
        var secondDigit: Int = -1
        var string = string

        while !string.isEmpty, (firstDigit < 0 || secondDigit < 0) {
            for (val, digit) in digits.enumerated() {
                if firstDigit == -1, string.hasPrefix(digit) {
                    string.removeFirst(digit.count)
                    firstDigit = val + 1
                }
                if secondDigit == -1, string.hasSuffix(digit) {
                    string.removeLast(digit.count)
                    secondDigit = val + 1
                }
            }

            if firstDigit == -1, let int = Int(String(string.removeFirst())) {
                firstDigit = int
            }
            if !string.isEmpty, secondDigit == -1, let int = Int(String(string.removeLast())) {
                secondDigit = int
            }
        }
        if firstDigit == -1 { firstDigit = secondDigit }
        if secondDigit == -1 { secondDigit = firstDigit }
        return firstDigit == -1 ? firstDigit : firstDigit * 10 + secondDigit
    }

    public static let digits = ["one","two","three","four","five","six","seven","eight","nine"]
